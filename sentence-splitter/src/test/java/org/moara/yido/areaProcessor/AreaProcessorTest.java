package org.moara.yido.areaProcessor;
...
public class AreaProcessorTest {
    ...
    @Test
    public void exceptionAreaProcessorTest() {
        ExceptionAreaProcessor exceptionAreaProcessor = new ExceptionAreaProcessor(BasicRoleManager.getRoleManager());
        ...
        int[] beforeAnswer = {832, 1043, 1188};
        int[] afterAnswer = {841, 1085, 1221};
        int answerIndex = 0;
        int areaLength = 3;
        for(int i = 0 ; i < inputData.length() ; i++) {
            Area searchArea = new Area(i, i + areaLength);
            exceptionAreaProcessor.find(inputData);
            if(searchArea.getStart() != exceptionAreaProcessor.avoid(searchArea).getStart()) {
                assertEquals(i, beforeAnswer[answerIndex]);
                i = exceptionAreaProcessor.avoid(searchArea).getStart();
    ...
    @Test
    public void TerminatorAreaProcessorTest() {
        RoleManager roleManager = BasicRoleManager.getRoleManager();
        ExceptionAreaProcessor exceptionAreaProcessor = new ExceptionAreaProcessor(roleManager);
        ...
        int index = 0;
        for (Integer i : terminatorAreaProcessor.find(inputData, exceptionAreaProcessor)) {
