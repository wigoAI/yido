...
//        String data = "다.다.다.다.다.다.다.다.다.다.다.(다.다.다.다.다.다.다.다.다.)다.다.다.다.다.다.다.다.다.다.다.다.다.다.다.다.다.다.다.다.다.다.다.다.다.다.다.(다.다.다.다.다.다.다.다.다.)다.다.다.다.다.다.다.다.";
        ...
        double splitWithList = 0;
        double splitWithArray = 0;
        for (int k = 0 ; k < 100 ; k++) {
            ...
            splitWithList += ((end - start) / 1000.0);


            start = System.currentTimeMillis();
            int[] splitPointArray = terminatorAreaProcessor.findByTextWithArray(data, exceptionAreas);
            end = System.currentTimeMillis();
            int compareSize2 = splitPointArray.length;

            System.out.println("Array processing 실행 시간 : " + (end - start) / 1000.0);
            splitWithArray += ((end - start) / 1000.0);

            if (compareSize != compareSize2) {
                Arrays.stream(splitPointArray).forEach(System.out::println);
                break;
            }
        }

        System.out.println("text loop : " + (splitWithList / 100) );
        System.out.println("text parallel loop : " + (splitWithArray / 100) );
    }
}
