...
//        }
    ...
    private List<Integer> findByTextLoopParallel(String text, List<Area> exceptionAreas) {
        List<Area> processingArea = new ArrayList<>();

        for (int i = 0; i < text.length() - minResultLength; i++) {
            for (int conditionLength : conditionLengths) {
                processingArea.add(new Area(i, i + conditionLength));
            }
        }

        TreeSet<Integer> splitPoints = processingArea.stream().parallel()
                .filter(area -> area.getEnd() <= text.length() - minResultLength).filter(area -> {

            String targetString = text.substring(area.getBegin(), area.getEnd());

            if (splitConditionValues.contains(targetString)) {

                SplitCondition targetSplitCondition = getSplitConditionByValue(targetString);

                return isValidCondition(text, targetSplitCondition, area.getBegin());
            ...
            SplitCondition targetSplitCondition = getSplitConditionByValue(targetString);
            if (targetSplitCondition.getSplitPosition() == 'F') {
                splitPoint = area.getBegin();
            } else { // splitPosition == 'B'
                int additionalSignLength = getAdditionalSignLength(area.getEnd(), text);
                splitPoint = area.getEnd() + additionalSignLength;
            }
        ...
        Set<Integer> removePoints = new HashSet<>();
        NavigableSet<Integer> treeReverse = splitPoints.descendingSet();
        Iterator<Integer> iterator = treeReverse.iterator();

        if (!iterator.hasNext()) {
            return new ArrayList<>(splitPoints);
        }
        int standardSplitPoint = iterator.next();
        while (iterator.hasNext()) {
            int targetSplitPoint = iterator.next();

            if (!removePoints.contains(targetSplitPoint)) {
                if (targetSplitPoint > standardSplitPoint - minResultLength) {
                    removePoints.add(targetSplitPoint);
                } else {
        ...
        splitPoints.removeAll(removePoints);
...
//        terminatorAreaProcessor.addSplitConditions(additionalSplitCondition);
            ...
            System.out.println("size " + splitPoints.size());
...
//        splitPoints.forEach(System.out::println);
            ...
            textLoopTime += ((end - start) / 1000.0);


            start = System.currentTimeMillis();
            splitPoints = terminatorAreaProcessor.findByTextLoopParallel(data, exceptionAreas);
            end = System.currentTimeMillis();
            System.out.println("size " + splitPoints.size());

//        splitPoints.forEach(System.out::println);
            System.out.println("text loop parallel 실행 시간 : " + (end - start) / 1000.0);
            textParallelTime += ((end - start) / 1000.0);
            if (splitPoints.size() != compareSize) {
                splitPoints.forEach(System.out::println);
                break;
            }
        }
...
//        end = System.currentTimeMillis();
//        System.out.println("size " + splitPoints2.size());
////        splitPoints2.forEach(System.out::println);
//        System.out.println("rule loop 실행 시간 : " + (end - start) / 1000.0);

        System.out.println("text loop : " + (textLoopTime / 100) );
        System.out.println("text parallel loop : " + (textParallelTime / 100) );
    }
}
